...
DATA = np.genfromtxt('buoyancy_binary_interactions_Townsend.txt',dtype='str')
...
beta=1.3
al=np.arctan(beta)
Mf1 = 20/(6*1*np.pi*1*10)
Mf2 = Mf1/1.10
V=np.array(V)
V=V/Mf1
R=3.3

V_x_0=[V[i][0][2] for i in range(0,99)]
V_x_mono_0=[V[i][0][2] for i in range(99,198)]
V_x_2=[V[i][0][2] for i in range(198,297)]
V_x_mono_2=[V[i][0][2] for i in range(297,396)]
V_x_5=[V[i][0][2] for i in range(396,495)]
V_x_mono_5=[V[i][0][2] for i in range(495,594)]
V_x_6=[V[i][0][2] for i in range(594,693)]
V_x_mono_6=[V[i][0][2] for i in range(693,792)]
print(V_x_mono_0)
...
for i in range(len(G)):
    ...
    plt.plot(lamda,An/np.abs(An[-1]), marker = '', linestyle='-', color=cmap(0.9+i/100), label = 'Analytical side velocity')
plt.plot(lamda, V_x_0/np.abs(V_x_0[-1]), marker = 'v', linestyle=' ', color=cmap(0.1), label = 'side velocity')
plt.plot(lamda, V_x_mono_0/np.abs(V_x_mono_0[-1]), marker = '.', linestyle=' ', color=cmap(0.4), label = 'side velocity monodisperse')
plt.plot(lamda, V_x_2/np.abs(V_x_2[-1]), marker = 's', linestyle=' ', color=cmap(0.1), label = 'side velocity')
plt.plot(lamda, V_x_mono_2/np.abs(V_x_mono_2[-1]), marker = '.', linestyle=' ', color=cmap(0.4), label = 'side velocity monodisperse')
plt.plot(lamda, V_x_5/np.abs(V_x_5[-1]), marker = 'v', linestyle=' ', color=cmap(0.1), label = 'side velocity')
plt.plot(lamda, V_x_mono_5/np.abs(V_x_mono_5[-1]), marker = '.', linestyle=' ', color=cmap(0.4), label = 'side velocity monodisperse')
plt.plot(lamda, V_x_6/np.abs(V_x_6[-1]), marker = 'v', linestyle=' ', color=cmap(0.1), label = 'side velocity')
plt.plot(lamda, V_x_mono_6/np.abs(V_x_mono_6[-1]), marker = '.', linestyle=' ', color=cmap(0.4), label = 'side velocity monodisperse')
...
plt.tight_layout()
np.savez('Townsend_buoyancy_binary_velocity_'+str(timestamp), V_x_mono_0,V_x_0,V_x_mono_2, V_x_2, V_x_mono_5, V_x_5, V_x_mono_6, V_x_6, lamda)
plt.savefig('output/figure'+str(timestamp) +'Two sphere distance.pdf', format='pdf', dpi=300 )
plt.show()
